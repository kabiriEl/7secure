...
if st.session_state.is_loading:
    with st.spinner("⏳ Génération de la newsletter en cours..."):
        try:
            ...
            st.session_state.newsletter_html = data.get("html", "")
            st.session_state.error_message = None
            ...
            st.session_state.error_message = (
                " Délai d'attente dépassé. Génération trop longue."
            )
            st.session_state.is_loading = False
...
if st.session_state.newsletter_content:
    ...
    tab1, tab2 = st.tabs(["🎨 Vue Formatée (HTML)", "📝 Texte Brut"])
    ...
    with tab1:
        ...
        if st.session_state.newsletter_html:
            ...
            st.info("Version HTML non disponible. Veuillez régénérer la newsletter.")
    
    with tab2:
        # Display raw text version
        st.markdown("""
    ...
    with col3:
        ...
        if st.session_state.newsletter_html:
            html_filename = f"newsletter_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
            st.download_button(
